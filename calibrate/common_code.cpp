                             ...
                             const char * wname)
{
    ...
    if(was_found){
        ...
        //creamos nuestro criterio y se lo pasamos a la funcion posterior
        cv::TermCriteria criteria = cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT, 40, 0.001);
        cv::cornerSubPix(aux, corner_points, cv::Size(5,5), cv::Size(-1, -1), criteria);

    }//size(5,5) porque es el ejemplo y (-1,-1) porque afirmamos que no hay zonas muertas
                      ...
                      std::vector<cv::Mat>* tvecs)
{
    ...
    //Sin esto da segmentation fault al intentar calibrar cuando no 
    //nos llegan rvecs ni tvecs
                            ...
                            double fps)
{
    ...
    cv::Mat map1, map2;
    cv::Mat input_frame, output_frame, r;
    ...
    cv::initUndistortRectifyMap(camera_matrix, dist_coeffs, r, camera_matrix, input_frame.size(), CV_32FC1, map1, map2);
